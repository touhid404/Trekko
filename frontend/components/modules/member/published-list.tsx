...
import { Loader2, Eye, MapPin, CheckCircle2 } from "lucide-react"
import {
  Table,
  TableBody,
  TableCell,
  ...
  if (!data?.data || data.data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-24 text-center shadow-sm">
        <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-slate-50 text-emerald-500">
          ...
          <TableBody>
            {guides.map((guide) => (
              <TableRow key={guide.id}>
                ...
                <TableCell>
                  <Badge variant="outline" className="bg-emerald-50 text-emerald-600 border-emerald-200 font-bold uppercase tracking-widest text-[9px] px-3 py-1">
        ...
        <div className="flex flex-col items-center gap-4 pt-4">
          ...
          {meta.total > 0 && (
            <div className="text-xs font-semibold text-slate-400">
              ...
              {Math.min(currentPage * meta.limit, meta.total)} of {meta.total} published guides
            </div>
          )}
        </div>
      )}
